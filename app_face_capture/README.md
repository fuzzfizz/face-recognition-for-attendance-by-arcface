...
- **Server API Ingestion**: Uploads photos directly to the `ai_server` registration endpoint for face quality validation and queueing.
...
- **Settings Screen**: Easily configure the server's base URL.
- **Local Settings Persistence**: Keeps the server URL saved locally via `SharedPreferences`.
...
│   │   └── services/                  # Face API HTTP service
│   ├── presentation/
...
├── pubspec.yaml                       # App dependencies (shared_preferences, camera, etc.)
...
Start the app, navigate to **Settings** (gear icon in the top right), and configure the **Server Base URL** to point to your running `ai_server` URL (e.g. `http://10.0.2.2:8000` for Android Emulator, or the local IP of your server).
