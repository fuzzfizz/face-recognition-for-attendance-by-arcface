import datetime
import os
from sqlalchemy import create_engine, Column, Integer, String, Float, DateTime, ForeignKey, Text, Enum as SqlEnum
...
class RegistrationQueue(Base):
    """
    Queue for async face registration processing.
    When a student uploads photos via /register, records are created here
    with status='pending'. A background worker (or /train-now trigger) processes
    them by extracting face embeddings.
    """
